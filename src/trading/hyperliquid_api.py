...
class HyperliquidAPI:
    ...
    def __init__(self):
        """Build Info client always. `Exchange` client only when NOT in paper mode.
        Paper mode: wallet optional, no order submission possible from this class.
        """
        self._meta_cache = None
        self._hip3_meta_cache = {}
        self._paper = bool(CONFIG.get("paper_trading_mode"))
        ...
        elif CONFIG.get("mnemonic"):
            Account.enable_unaudited_hdwallet_features()
            self.wallet = Account.from_mnemonic(CONFIG["mnemonic"])
        elif not self._paper:
            raise ValueError("Real-money mode needs HYPERLIQUID_PRIVATE_KEY or MNEMONIC.")

        self.account_address = CONFIG.get("hyperliquid_vault_address")
        self.query_address = self.account_address or (self.wallet.address if self.wallet else None)
        self._build_clients()

    def _build_clients(self):
        """Always build Info (read-only). Exchange (order-placing) only in live mode."""
        self.info = Info(self.base_url)
        if self._paper or self.wallet is None:
            self.exchange = None
        else:
            self.exchange = Exchange(self.wallet, self.base_url, account_address=self.account_address)

    def _require_exchange(self, action: str):
        if self.exchange is None:
            raise RuntimeError(
                f"Order attempt blocked: {action}. PAPER_TRADING_MODE is on "
                "and no real Exchange client exists. Route orders through PaperBroker."
            )
